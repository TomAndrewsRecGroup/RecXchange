 ...
 * 
 * Called by: GHL AI Agent when recruiter wants role recommendations
 * 
 * Webhook URL: https://recxchange.io/api/ghl/ai-actions/send-roles
 * Method: POST
 * 
...
export async function POST(req: NextRequest) {
  console.log('[GHL AI Action] Send Roles webhook triggered');
  ...
  try {
    ...
    console.log('[GHL AI Action] Calling send-3-roles API');
    ...
    console.log('[GHL AI Action] Roles email sent:', sendRolesData);
    ...
    if (contactId && conversationId) {
      try {
        ...
        console.error('[GHL AI Action] Failed to send confirmation:', msgError);
      }
    }

    // Return data for GHL AI to use in conversation
    return NextResponse.json({
      success: true,
      data: {
