 ...
 * 
 * Called by: Groq AI Agent when hiring manager wants to schedule a call
 * 
 * Webhook URL: https://recxchange.io/api/groq/ai-actions/schedule-call
 * Method: POST
 * 
...
export async function POST(req: NextRequest) {
  console.log('[Groq AI Action] Schedule Call webhook triggered');
...
Click the link above to choose a time that works for you. I'll send a calendar invite immediately after you book!
    ...
    if (contactId && conversationId) {
      try {
        ...
        console.error('[Groq AI Action] Failed to send booking message:', msgError);
      }
    }
    ...
    if (contactId) {
      try {
        ...
        console.error('[Groq AI Action] Failed to create task:', taskError);
      }
    }
