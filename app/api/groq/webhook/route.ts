...
export async function POST(req: NextRequest) {
  try {
    ...
    console.log('[Groq Webhook] Received payload:', JSON.stringify(payload));
    ...
    if (contactId && visitorContactIds.has(contactId)) {
      console.log('[Groq Webhook] Ignoring visitor echo for contactId:', contactId);
      return NextResponse.json({ received: true });
    }

    // If direction is explicitly inbound, also skip
    if (direction && direction !== 'outbound') {
      console.log('[Groq Webhook] Ignoring non-outbound message, direction:', direction);
      return NextResponse.json({ received: true });
    }
    ...
    if (!conversationId) {
      console.warn('[Groq Webhook] Could not resolve conversationId for contactId:', contactId);
      return NextResponse.json({ received: true });
    }
