 ...
 * 
 * Sends weekly RECRUITER funnel report via SendGrid
 * Includes: GHL tier signups + recruiter funnel + quick action stats
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!SENDGRID_API_KEY) {
      console.error('[Recruiter Funnel] SENDGRID_API_KEY not configured');
    ...
    // Calculate recruiter funnel metrics
    ...
    if (GHL_API_KEY && GHL_LOCATION_ID) {
      try {
        ...
        console.log('[Recruiter Funnel] GHL conversion data fetched:', ghlData);
      } catch (error) {
        console.error('[Recruiter Funnel] Failed to fetch GHL data:', error);
      }
    }

    // Count quick action events
    const quickActionCount = events.filter(
      (e: any) => e.event === 'quick_action_submitted' && 
                  e.properties?.action_type === 'match_candidate'
    ).length;

    // Generate HTML email content
    const htmlContent = generateRecruiterEmail(recruiterMetrics, ghlData, quickActionCount);
    ...
    const emailResponse = await fetch('https://api.sendgrid.com/v3/mail/send', {
      ...
      body: JSON.stringify({
        personalizations: [{
          to: [{ email: EMAIL_TO }],
          subject: `💼 Recruiter Funnel Report - ${new Date().toLocaleDateString()}`
        }],
    ...
    if (!emailResponse.ok) {
      ...
      console.error('[Recruiter Funnel] SendGrid error:', errorText);
      return NextResponse.json(
    ...
    console.error('[Recruiter Funnel] Error:', error);
    return NextResponse.json(
      { error: 'Failed to send recruiter funnel email' },
      { status: 500 }
    );
  }
}
...
<head>
  <style>
    ...
    .header .subtitle { margin: 12px 0 0 0; color: #00ffff; font-size: 18px; font-weight: 600; }
    ...
    .tier-count { font-size: 36px; font-weight: 900; background: linear-gradient(135deg, #00ffff, #c71df1); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .tier-card.has-signups .tier-count { background: linear-gradient(135deg, #22c55e, #00ffff); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    ...
    .summary-value { font-size: 48px; font-weight: 900; background: linear-gradient(135deg, #00ffff, #c71df1); -webkit-background-clip: text; -webkit-text-fill-color: transparent; line-height: 1; }
    ...
    .stage-number { flex-shrink: 0; width: 40px; height: 40px; background: linear-gradient(135deg, #00ffff, #c71df1); border-radius: 50%; display: flex; align-items: center; justify-content: center; color: #000; font-weight: 900; font-size: 18px; }
    ...
    .stage-count { font-size: 32px; font-weight: 900; background: linear-gradient(135deg, #00ffff, #c71df1); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
    .progress-bar-container { background: #1a1a1a; height: 12px; border-radius: 6px; overflow: hidden; margin-bottom: 10px; }
    .progress-bar { height: 100%; background: linear-gradient(90deg, #00ffff, #c71df1); border-radius: 6px; }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <div class="subtitle">💼 Recruiter Focus</div>
    ...
    <div class="content">
      ...
      ${generateFunnelHTML(metrics, quickActionCount)}
    </div>
    
    <div class="footer">
      <p><strong>RecXchange Analytics</strong> • Recruiter Weekly Report</p>
...
function generateFunnelHTML(metrics: FunnelMetrics, quickActionCount: number): string {
  ...
  return `
    <div class="funnel">
      ...
      <div class="summary-grid">
        ...
        <div class="summary-card"><div class="summary-label">Conversion Rate</div><div class="summary-value">${metrics.conversionRate}%</div><div class="summary-subtext">signed up</div></div>
        <div class="summary-card"><div class="summary-label">Converted</div><div class="summary-value">${metrics.stages[metrics.stages.length - 1]?.count || 0}</div><div class="summary-subtext">new users</div></div>
      </div>
      ...
      ${quickActionCount > 0 ? `<div class="quick-action-stat">🎯 <strong>${quickActionCount}</strong> recruiters requested candidate matches via quick action form</div>` : ''}
    </div>
  `;
}
