  ...
  return (
    <main className="relative min-h-screen flex flex-col items-center pt-32 pb-20 px-6 overflow-x-hidden mesh-background">
      ...
      <section className="w-full grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
        ...
        {/* Option A: I need candidates (Cyan) */}
        <Link href="/recruiter-roles" className="group h-full">
          <motion.div
            ...
            animate="animate"
            whileHover="hover"
            className="h-full glass-card rounded-[3rem] p-10 flex flex-col justify-between border-cyan-400/10 relative overflow-hidden bg-white/[0.02]"
          >
            <div className="absolute -top-24 -left-24 w-64 h-64 bg-cyan-500/10 blur-[80px] rounded-full pointer-events-none" />

            <div className="relative z-10">
              <div className="w-12 h-12 rounded-2xl bg-cyan-500/10 border border-cyan-400/20 flex items-center justify-center text-cyan-400 mb-8 group-hover:scale-110 transition-transform">
              ...
              <div className="space-y-3">
                {["Instant AI Shortlisting", "Verified Recruiter Network", "Clients Protected"].map((item, i) => (
                  <div key={i} className="flex items-center gap-3 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                    <div className="w-1 h-1 rounded-full bg-cyan-400" /> {item}
                  </div>
                ))}
              </div>
            </div>

            <div className="mt-12 flex items-center justify-between pt-8 border-t border-cyan-400/10">
              <span className="text-[10px] font-black text-cyan-400 uppercase tracking-widest">80%+ Matched Candidates, straight to you.</span>
              <div className="w-8 h-8 rounded-full bg-cyan-400/10 flex items-center justify-center text-cyan-400 group-hover:translate-x-2 transition-transform">→</div>
            </div>
          </motion.div>
        </Link>

        {/* Option B: I need roles (Fuchsia) */}
        <Link href="/recruiters-with-candidates" className="group h-full">
          <motion.div
            ...
            animate="animate"
            whileHover="hover"
            className="h-full glass-card rounded-[3rem] p-10 flex flex-col justify-between border-fuchsia-400/10 relative overflow-hidden bg-white/[0.02]"
          >
            <div className="absolute -bottom-24 -right-24 w-64 h-64 bg-fuchsia-500/10 blur-[80px] rounded-full pointer-events-none" />

            <div className="relative z-10">
              <div className="w-12 h-12 rounded-2xl bg-fuchsia-500/10 border border-fuchsia-400/20 flex items-center justify-center text-fuchsia-400 mb-8 group-hover:scale-110 transition-transform">
                <svg width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              <div className="space-y-3">
                {["Collaborative Split Fees", "Automated SFAs", "Circle of Trust Privacy"].map((item, i) => (
                  <div key={i} className="flex items-center gap-3 text-[10px] font-bold text-gray-500 uppercase tracking-widest">
                    <div className="w-1 h-1 rounded-full bg-fuchsia-400" /> {item}
                  </div>
                ))}
              </div>
            </div>

            <div className="mt-12 flex items-center justify-between pt-8 border-t border-fuchsia-400/10">
              <span className="text-[10px] font-black text-fuchsia-400 uppercase tracking-widest">Data protected until you’re ready</span>
              <div className="w-8 h-8 rounded-full bg-fuchsia-400/10 flex items-center justify-center text-fuchsia-400 group-hover:translate-x-2 transition-transform">→</div>
            </div>
          </motion.div>
        </Link>
      </section>
