...
// Industry list for Send Me 3 Roles form
const INDUSTRIES = [
  'Accounting & Finance',
...
export default function HiringManagerHome() {
  const [sendRolesFormOpen, setSendRolesFormOpen] = useState(false);
  const [howItWorksFormOpen, setHowItWorksFormOpen] = useState(false);
  
  // Send Me 3 Roles form state
  ...
  const handleSendRolesSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (selectedIndustries.length === 0 || !rolesEmail || !rolesName) return;
    
    setRolesSubmitting(true);
    
    try {
      // Submit to Eavee/GHL with industry tags
      const response = await fetch('/api/submit-send-roles', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          tags: selectedIndustries // Tags for Eavee/GHL profile
        })
      });
    ...
    <main className="relative min-h-screen flex flex-col items-center pt-32 pb-20 px-6 overflow-x-hidden">
      ...
      <div className="relative z-10 w-full max-w-[1200px] flex flex-col items-center">
        ...
        <div className="w-full mb-16 flex flex-wrap justify-center gap-4">
          <button
            onClick={() => setSendRolesFormOpen(true)}
      ...
      {/* Send Me 3 Roles Modal */}
      {sendRolesFormOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/80 backdrop-blur-sm" onClick={() => setSendRolesFormOpen(false)}>
          <motion.div
          ...
          >
            <button
              onClick={() => setSendRolesFormOpen(false)}
            ...
            {rolesSuccess ? (
              ...
              <form onSubmit={handleSendRolesSubmit} className="space-y-6">
                <div>
                  ...
                  <input
                    ...
                    required
                    className="w-full bg-white/[0.03] border border-white/10 rounded-xl px-4 py-3 text-white outline-none focus:border-cyan-400/50 transition-all"
                    ...
                    <div className="flex flex-wrap gap-2 mb-4">
                      {selectedIndustries.map((industry) => (
                        <button
                          ...
                          className="px-3 py-1.5 rounded-full bg-cyan-400/10 border border-cyan-400/30 text-cyan-400 text-xs font-bold flex items-center gap-2 hover:bg-cyan-400/20 transition-all"
                        >
                          {industry}
                          <X size={14} />
                        </button>
                ...
                <button
                  type="submit"
                  disabled={rolesSubmitting || selectedIndustries.length === 0}
                ...
                >
                  ...
                  <span className="relative z-10 text-white flex items-center justify-center">
                    {rolesSubmitting ? 'Sending...' : 'Send Me 3 Roles'}
                  </span>
                </button>
              </form>
            )}
