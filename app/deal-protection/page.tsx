        ...
        <div className="flex flex-col gap-8 pb-10">
          ...
          <section className="glass-card p-10 md:p-14 rounded-[3.5rem] border-cyan-400/10 relative overflow-hidden bg-gradient-to-br from-cyan-400/[0.02] to-transparent shadow-2xl">
            <div className="relative z-10">
              ...
              <div className="relative flex flex-col md:flex-row items-center justify-between gap-12 md:gap-4 bg-black/40 p-10 rounded-[2.5rem] border border-cyan-400/10">
                ...
                {steps.map((step, i) => (
                  ...
                  >
                    <div className="w-12 h-12 rounded-2xl bg-black border border-cyan-400/30 flex items-center justify-center shadow-[0_0_20px_rgba(0,255,255,0.1)] hover:border-cyan-400 transition-colors">
                      <span className="text-xs font-black text-cyan-400">0{i + 1}</span>
                    </div>
                    <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <motion.section
              ...
              className="glass-card p-10 rounded-[2.5rem] border-cyan-400/10 bg-gradient-to-b from-cyan-400/[0.02] to-transparent"
            >
              ...
              <div className="space-y-4">
                {["Clear fees & splits", "Standardized language", "One source of truth"].map((item, i) => (
                  <div key={i} className="flex items-center gap-3 text-[10px] font-black uppercase tracking-widest text-gray-500">
                    <span className="w-1 h-1 rounded-full bg-cyan-400" /> {item}
                  </div>
                ))}
              </div>
            </motion.section>
            ...
            <motion.section
              whileHover={{ translateY: -5 }}
              className="glass-card p-10 rounded-[2.5rem] border-fuchsia-400/10 bg-gradient-to-b from-fuchsia-400/[0.02] to-transparent"
            >
              <div className="w-10 h-10 rounded-xl bg-fuchsia-500/10 flex items-center justify-center mb-8 border border-fuchsia-500/20">
                <svg className="w-5 h-5 text-fuchsia-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
              </div>
              ...
              <Link href="/roles" className="text-cyan-400 text-[10px] font-black hover:text-white transition-colors uppercase tracking-[0.2em] flex items-center gap-2">
          ...
          <section className="glass-card p-10 rounded-[2.5rem] border-cyan-400/10 flex flex-col md:flex-row items-center justify-between gap-8 bg-black/20">
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {["Agreed Resolution", "Transparency", "Data Privacy"].map((text, i) => (
                <span key={i} className="px-5 py-2.5 rounded-xl border border-cyan-400/10 bg-cyan-400/[0.03] text-[9px] font-black text-gray-400 uppercase tracking-[0.2em]">
                  {text}
                </span>
              ))}
            </div>
