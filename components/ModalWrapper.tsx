    ...
    <AnimatePresence>
      {isOpen && (
        <motion.div
          ...
          className="fixed inset-0 z-[100] flex items-center justify-center overflow-y-auto bg-black/90"
          style={{ 
            top: '64px', // Below header (adjust if header height changes)
            bottom: '0',  // Above footer
          }}
          onClick={handleBackdropClick}
        >
          <div className="w-full min-h-full flex items-center justify-center p-4 sm:p-6 md:p-8">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 20 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 20 }}
              transition={{ duration: 0.2 }}
              className={`w-full ${maxWidthClasses[maxWidth]} my-auto rounded-2xl sm:rounded-3xl md:rounded-[2.5rem] relative ${
                className
              }`}
              style={{
                background: '#0a0a0a',
